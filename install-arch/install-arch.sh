...
validate_timezone() {
  local selected_timezone="$1"
  [[ "$selected_timezone" =~ ^[A-Za-z0-9+._-]+(/[A-Za-z0-9+._-]+)+$ ]] &&
    [ -f "/usr/share/zoneinfo/$selected_timezone" ]
}
...
verify_primary_user() {
  ...
  if [ "$account_home" != "$expected_home" ] || [ "$account_shell" != "$user_shell" ]; then
...
ensure_primary_user() {
  ...
  if account_record="$(in_target getent passwd "$selected_user")"; then
    ...
    in_target usermod -d "$expected_home" -g "$selected_user" \
      -aG docker,wheel -s "$user_shell" -p "$selected_password_hash" "$selected_user"
  else
    if target_group_record "$selected_user" >/dev/null 2>&1; then
      in_target useradd -m -g "$selected_user" -G docker,wheel \
        -s "$user_shell" -p "$selected_password_hash" "$selected_user"
    else
      in_target useradd -mU -G docker,wheel -s "$user_shell" \
...
if [ "$TEST_MODE" = "true" ]; then
  ...
  dialog_capture mode "install_mode" "menu" \
    ...
    "1" "Minimal" "2" "Workstation" "3" "VirtualBox" "4" "Bare" || exit 1
fi
...
if [ "$TEST_MODE" = "true" ]; then
  timezone="${TEST_MODE_TIMEZONE:-Asia/Shanghai}"
else
  debug_checkpoint "collecting_timezone"
  mapfile -t timezone_options < <(timedatectl list-timezones 2>/dev/null || find /usr/share/zoneinfo -maxdepth 2 -mindepth 2 -type f | sed 's|/usr/share/zoneinfo/||' | sort)
  if [ "${#timezone_options[@]}" -eq 0 ]; then
    echo "Error: No timezone data found."
    exit 1
  fi
  dialog_capture timezone "timezone" "menu" \
    "$DIALOG_MENU_HEIGHT" "$DIALOG_MENU_WIDTH" "$DIALOG_MENU_ROWS" \
    "${#timezone_options[@]}" \
    --clear --menu "Select timezone" \
    "$DIALOG_MENU_HEIGHT" "$DIALOG_MENU_WIDTH" "$DIALOG_MENU_ROWS" \
    "${timezone_options[@]}" || exit 1
fi
[ -z "$timezone" ] && echo "timezone cannot be empty" && exit 1
if ! validate_timezone "$timezone"; then
  echo "Error: Invalid timezone '$timezone'. Must be a valid path under /usr/share/zoneinfo (e.g. Asia/Shanghai, US/Pacific)."
  exit 1
fi

if [ "$TEST_MODE" = "true" ]; then
  mirror_source="${TEST_MODE_MIRROR:-arch}"
else
  debug_checkpoint "collecting_mirror_source"
  dialog_capture mirror_source "mirror_source" "menu" \
    "$DIALOG_MENU_HEIGHT" "$DIALOG_MENU_WIDTH" "$DIALOG_MENU_ROWS" "2" \
    --clear --menu "Select mirror source" \
    "$DIALOG_MENU_HEIGHT" "$DIALOG_MENU_WIDTH" "$DIALOG_MENU_ROWS" \
    "arch" "Arch official (archlinux.org)" \
    "china" "China mirrors (linuxmirrors.cn)" || exit 1
fi
if [ "$mirror_source" != "arch" ] && [ "$mirror_source" != "china" ]; then
  echo "Error: Invalid mirror source '$mirror_source'."
  exit 1
fi
...
if [ "$RESUME" = "false" ]; then
  ...
  if [ "$DRY_RUN" = "true" ]; then
    dry_run_msg "Would update mirrorlist (source=$mirror_source)"
  else
    if [ "$mirror_source" = "china" ]; then
      echo "Running linuxmirrors.cn mirror script..."
...
run_required_step "link /mnt/etc/localtime to $timezone timezone" \
  ensure_target_symlink /mnt "/usr/share/zoneinfo/$timezone" /etc/localtime
